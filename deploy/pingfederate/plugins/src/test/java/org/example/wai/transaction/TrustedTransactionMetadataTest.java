...
class TrustedTransactionMetadataTest {
  @Test void derivesTrustedValuesAndOverwritesCallerAssertions() {
    ...
    var metadata = new TrustedTransactionMetadata("spiffe://example.org/agent/demo",
        "urn:agent:demo", "system.whoami", () -> "server-id-" + sequence.incrementAndGet());
  ...
  @Test void rejectsWrongWorkloadAndUnknownPurpose() {
    ...
    assertThrows(IllegalArgumentException.class,
        () -> new TrustedTransactionMetadata("spiffe://example.org/agent/demo",
            "urn:agent:demo", "delete.everything", () -> "id"));
  }

  @Test void rejectsMalformedBindings() {
    assertThrows(IllegalArgumentException.class,
        () -> new TrustedTransactionMetadata("https://example.org/agent/demo",
