...
image-push-tts-adapter image-push-strict-mcp-gateway image-push-strict-demo-mcp-server image-push-strict-demo-api image-push-web-app image-push-audit-collector image-push-spire-jwks-refresher: image-source-check
...
verify-workbench-public-surface:
	pwsh -NoProfile -File scripts/verify-workbench-public-surface.ps1

vault-configure-strict:
	pwsh -NoProfile -File scripts/configure-wai-strict-vault-role.ps1
...
pf-update-browser-redirect:
	$(PYTHON_RUN) -EnvFile .env.local -ScriptPath deploy/pingfederate/scripts/update_browser_redirect.py
